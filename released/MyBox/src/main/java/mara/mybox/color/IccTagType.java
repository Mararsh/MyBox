...
//        }
    ...
    public static String textDescriptionFullDisplay(Map<String, Object> values) {
        String s = "";
        s += Languages.message("AsciiLength") + ": " + values.get("AsciiLength") + "\n";
        s += values.get("Ascii") + "\n\n";

        try {
            if (values.get("UnicodeCode") != null) {
                s += Languages.message("UnicodeCode") + ": " + values.get("UnicodeCode") + "  ";
                s += Languages.message("UnicodeLength") + ": " + values.get("UnicodeLength") + "\n";
                s += values.get("Unicode") + "\n\n";
            }

            if (values.get("ScriptCodeCode") != null) {
                s += Languages.message("ScriptCodeCode") + ": " + values.get("ScriptCodeCode") + "  ";
                s += Languages.message("ScriptCodeLength") + ": " + values.get("ScriptCodeLength") + "\n";
    ...
    public static String XYZNumberDisplay(double[][] values) {
        ...
        for (int i = 0; i < values.length; ++i) {
            ...
            double[] row = values[i];
            s += row[0] + AppValues.Indent + row[1] + AppValues.Indent + row[2];
        }
        return s;
    }
    ...
    public static String viewingConditionsDisplay(Map<String, Object> values) {
        String s = "";
        double[] illuminant = (double[]) values.get("illuminant");
        s += Languages.message("Illuminant") + ": "
                + illuminant[0] + AppValues.Indent + illuminant[1] + AppValues.Indent + illuminant[2] + "\n";
        double[] surround = (double[]) values.get("surround");
        s += Languages.message("Surround") + ": "
                + surround[0] + AppValues.Indent + surround[1] + AppValues.Indent + surround[2] + "\n";
        String type = (String) values.get("illuminantType");
        s += Languages.message("Type") + ": " + type;
        return s;
    }
    ...
    public static String measurementDisplay(Map<String, Object> values) {
        String s = "";
        String observer = (String) values.get("observer");
        s += Languages.message("Observer") + ": " + observer + "\n";
        double[] tristimulus = (double[]) values.get("tristimulus");
        s += Languages.message("Tristimulus") + ": "
                + tristimulus[0] + AppValues.Indent + tristimulus[1] + AppValues.Indent + tristimulus[2] + "\n";
        String geometry = (String) values.get("geometry");
        s += Languages.message("Geometry") + ": " + geometry + "\n";
        double flare = (double) values.get("flare");
        s += Languages.message("Flare") + ": " + flare + "\n";
        String type = (String) values.get("illuminantType");
        s += Languages.message("Type") + ": " + type;
        return s;
    }
    ...
    public static String lutDisplay(Map<String, Object> values) {
        String s = "";
        s += Languages.message("InputChannelsNumber") + ": " + values.get("InputChannelsNumber") + "  ";
        s += Languages.message("OutputChannelsNumber") + ": " + values.get("OutputChannelsNumber") + "  ";
        if (values.get("type") == null) {
            s += Languages.message("Type") + ": " + Languages.message("NotDecoded") + "\n";
        } else {
            String type = (String) values.get("type");
            s += Languages.message("Type") + ": " + type + "\n";
            if (type.equals("lut8") || type.equals("lut16")) {
                s += Languages.message("GridPointsNumber") + ": " + values.get("GridPointsNumber") + "\n";
                s += Languages.message("Matrix") + ": \n";
                s += AppValues.Indent + values.get("e1") + AppValues.Indent + values.get("e2") + AppValues.Indent + values.get("e3") + "\n";
                s += AppValues.Indent + values.get("e4") + AppValues.Indent + values.get("e5") + AppValues.Indent + values.get("e6") + "\n";
                s += AppValues.Indent + values.get("e7") + AppValues.Indent + values.get("e8") + AppValues.Indent + values.get("e9") + "\n";

                if (type.equals("lut16")) {
                    s += Languages.message("InputTablesNumber") + ": " + values.get("InputTablesNumber") + "   ";
                    s += Languages.message("OutputTablesNumber") + ": " + values.get("OutputTablesNumber") + "\n";
                }
                s += Languages.message("InputTables") + ": " + "\n";
                List<List<Object>> InputTables = (List<List<Object>>) values.get("InputTables");
                for (List<Object> input : InputTables) {
                    s += AppValues.Indent;
                    for (Object v : input) {
                        s += v + AppValues.Indent;
                    }
                    s += "\n";
                }
                if (values.get("InputTablesTruncated") != null) {
                    s += AppValues.Indent + "-----" + Languages.message("Truncated") + "-----" + "\n";
                }
                s += Languages.message("CLUTTables") + ": " + "\n";
                ...
                s += Languages.message("OutputTables") + ": " + "\n";
