...
public class ImageTextController extends BaseController {
    ...
    public void initControls() {
        try {
            ...
            htmlEditor.addEventHandler(DragEvent.DRAG_EXITED, new EventHandler<InputEvent>() {
                @Override
    ...
    public void init(ImageManufactureRichTextController parent) {
        ...
        Object contents = WebViewTools.getHtml(parent.webEngine);
        if (contents != null) {
            htmlEditor.setHtmlText((String) contents);
        } else {
            htmlEditor.setHtmlText(Languages.message("ImageTextComments"));
        }
