...
//            }
            if (metaData.containsKey("ResolutionUnit")) {
                ...
                if (BaselineTIFFTagSet.RESOLUTION_UNIT_CENTIMETER == unit) {
                    info.setwDensity(ImageValueTools.dpcm2dpi(x));
                    info.sethDensity(ImageValueTools.dpcm2dpi(y));
                } else {
                    info.setwDensity(x);
                    info.sethDensity(y);
                }
            ...
            try (ImageOutputStream out = ImageIO.createImageOutputStream(targetFile)) {
                ...
                for (int i = 0; i < rows.size() - 1; i++) {
                    ...
                    for (int j = 0; j < cols.size() - 1; j++) {
                        ...
                        if (!imageInformation.isIsSampled()) {
                            bufferedImage = ImageConvertTools.cropImage(wholeSource, x1, y1, x2, y2);
                        } else {
