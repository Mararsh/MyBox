...
public class ImageTools {
    ...
    public static Image addText(Image image, String textString,
        ...
        BufferedImage target = ImageConvert.addText(source, textString,
                font, ImageTools.colorConvert(color), x, y, transparent, shadow, angle, isOutline);
        Image newImage = SwingFXUtils.toFXImage(target, null);
        return newImage;
    }
    ...
    public static Image addArc(Image image, int arc, Color bgColor) {
        ...
        BufferedImage source = SwingFXUtils.fromFXImage(image, null);
        BufferedImage target = ImageConvert.addArc(source, arc, ImageTools.colorConvert(bgColor));
        Image newImage = SwingFXUtils.toFXImage(target, null);
        return newImage;
    }
...
//            logger.error(e.toString());
    ...
    public static Image addShadow(Image image, int shadow, Color color) {
        ...
        BufferedImage source = SwingFXUtils.fromFXImage(image, null);
        BufferedImage target = ImageConvert.addShadow(source, shadow, ImageTools.colorConvert(color));
        Image newImage = SwingFXUtils.toFXImage(target, null);
        return newImage;
    }
...
//            logger.error(e.toString());
    ...
    public static Image indicateSplit(Image image,
        ...
        BufferedImage target = ImageScopeTools.indicateSplit(source, rows, cols,
                ImageTools.colorConvert(lineColor), lineWidth, showSize, scale);
        Image newImage = SwingFXUtils.toFXImage(target, null);
        return newImage;
    }
...
//                logger.debug(imageWidth + " " + imageHeight);
    ...
    public static Image blurBottomMargin(Image image, int size) {
        ...
        for (int x = 0; x < image.getWidth(); x++) {
            for (int y = 0; y < height; y++) {
                Color color = pixelReader.getColor(x, y);
                if (color.equals(Color.TRANSPARENT) || y <= height - size) {
                    pixelWriter.setColor(x, y, color);
                    continue;
                }
    ...
    public static Image blurLeftMargin(Image image, int size) {
        ...
        for (int y = 0; y < image.getHeight(); y++) {
            for (int x = 0; x < width; x++) {
                Color color = pixelReader.getColor(x, y);
                if (color.equals(Color.TRANSPARENT) || x <= width - size) {
                    pixelWriter.setColor(x, y, color);
                    continue;
                }
    ...
    public static int calculateColorDistance2(Color color1, Color color2) {
        int redDiff = (int) (color1.getRed() - color2.getRed()) * 255;
        int greenDiff = (int) (color1.getGreen() - color2.getGreen()) * 255;
        int blueDiff = (int) (color1.getBlue() - color2.getBlue()) * 255;
