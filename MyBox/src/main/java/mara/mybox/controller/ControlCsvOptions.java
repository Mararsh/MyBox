...
public class ControlCsvOptions extends BaseController {
    ...
    public void setControls(String name) {
        try {
            ...
            withNamesCheck.setSelected(AppVariables.getUserConfigBoolean(baseName + "Names", true));
            withNamesCheck.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue ov, Boolean oldValue, Boolean newValue) {
                    AppVariables.setUserConfigValue(baseName + "Names", newValue);
                }
            });
            ...
            delimiterGroup.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
                ...
                public void changed(ObservableValue ov, Toggle oldValue, Toggle newValue) {
                    ...
                    AppVariables.setUserConfigInt(baseName + "Delimiter", delimiter);
                }
            });
            delimiter = (char) (AppVariables.getUserConfigInt(baseName + "Delimiter", ','));
            switch (delimiter) {
                case ',':
                    commaRadio.fire();
                    break;
