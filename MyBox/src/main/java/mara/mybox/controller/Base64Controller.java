...
public class Base64Controller extends BaseController {
    ...
    public Base64Controller() {
        baseTitle = AppVariables.message("Base64Conversion");
    }
    ...
    protected void checkConvert() {
        try {
            ...
            FxmlControl.refreshStyle(inputBox);
        } catch (Exception e) {
            MyBoxLog.error(e.toString());
        }
    }
                ...
                protected boolean handle() {
                    try {
                        if (textRadio.isSelected()) {
                            ...
                            Base64.Encoder encoder = Base64.getEncoder();
                            byte[] bytes = FileTools.readBytes(sourceFile);
                            bytesLen = bytes.length;
                            results = encoder.encodeToString(bytes);
                            ...
                            Base64.Decoder decoder = Base64.getDecoder();
                            byte[] bytes = decoder.decode(FileTools.readTexts(sourceFile));
                            bytesLen = bytes.length;
                            results = new String(bytes, charset);
                        }
                        return true;
                ...
                protected boolean handle() {
                    try {
                        if (textRadio.isSelected()) {
                            ...
                            if (FileTools.writeFile(file, results) != null) {
                            ...
                            byte[] bytes = decoder.decode(inputArea.getText());
                            bytesLen = bytes.length;
                            if (FileTools.writeFile(file, bytes)) {
                                recordFileWritten(file);
                                return true;
                            } else {
                                return false;
