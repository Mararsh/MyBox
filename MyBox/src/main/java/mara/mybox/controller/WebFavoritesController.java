...
public class WebFavoritesController extends BaseDataTableController<WebFavorite> {
    ...
    public WebFavoritesController() {
        baseTitle = AppVariables.message("WebFavorites");
    }
    ...
    protected void checkSelected() {
        ...
        int selectedSize = tableView.getSelectionModel().getSelectedIndices().size();
        deleteButton.setDisable(selectedSize == 0);
        copyButton.setDisable(selectedSize == 0);
        moveDataButton.setDisable(selectedSize == 0);
        selectedLabel.setText(message("Selected") + ": " + selectedSize);
    }
    ...
    protected void downloadIcon() {
        synchronized (this) {
            ...
            SingletonTask updateTask = new SingletonTask<Void>() {
                ...
                @Override
                protected boolean handle() {
                    iconFile = HtmlTools.readIcon(address, true);
