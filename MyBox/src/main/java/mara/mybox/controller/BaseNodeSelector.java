...
public abstract class BaseNodeSelector<P> extends BaseController {
    ...
    public void initTree() {
        treeView.setCellFactory(p -> new TreeCell<P>() {
            ...
            public void updateItem(P item, boolean empty) {
                ...
                if (tips != null) {
                    FxmlControl.setTooltip(this, tips);
                }
            }
        });
    ...
    protected void popNodeMenu(MouseEvent event, TreeItem<P> selected) {
        ...
        if (manageMode) {
            ...
            menu = new MenuItem(message("CopyNodeAndContents"));
            menu.setOnAction((ActionEvent menuItemEvent) -> {
                copyNode(false);
            });
            menu.setDisable(isRoot(node.getValue()));
            ...
            menu = new MenuItem(message("CopyNodeContents"));
            menu.setOnAction((ActionEvent menuItemEvent) -> {
                copyNode(true);
            });
            menu.setDisable(isRoot(node.getValue()));
    ...
    protected void addNode() {
        synchronized (this) {
            ...
            String name = FxmlControl.askValue(getBaseTitle(), chainName, message("Add"), message("Node") + "m");
    ...
    protected void deleteNode() {
        synchronized (this) {
            ...
            if (isRoot) {
                if (!FxmlControl.askSure(getBaseTitle(), message("Delete"), message("SureDeleteAll"))) {
                    return;
                }
            } else {
                String chainName = chainName(targetItem);
                if (!FxmlControl.askSure(getBaseTitle(), chainName, message("Delete"))) {
                    return;
                }
            }
            task = new SingletonTask<Void>() {
    ...
    protected void renameNode() {
        synchronized (this) {
            ...
            String name = FxmlControl.askValue(getBaseTitle(), chainName, message("RenameNode"), name(book) + "m");
            if (name == null || name.isBlank()) {
                return;
            }
            if (name.contains(nodeSeparator)) {
