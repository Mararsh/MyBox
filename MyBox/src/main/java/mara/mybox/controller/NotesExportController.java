...
import static mara.mybox.tools.HtmlTools.Indent;
...
public class NotesExportController extends BaseTaskController {
    ...
    public NotesExportController() {
        baseTitle = AppVariables.message("NotesExport");
    }
    ...
    public void initControls() {
        try {
            ...
            timeCheck.setSelected(AppVariables.getUserConfigBoolean(baseName + "Time", false));
            timeCheck.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> v, Boolean oldV, Boolean newV) {
                    AppVariables.setUserConfigValue(baseName + "Time", timeCheck.isSelected());
                }
            });

            textsCheck.setSelected(AppVariables.getUserConfigBoolean(baseName + "Texts", true));
            textsCheck.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> v, Boolean oldV, Boolean newV) {
                    AppVariables.setUserConfigValue(baseName + "Texts", textsCheck.isSelected());
                }
            });
            ...
            framesetCheck.setSelected(AppVariables.getUserConfigBoolean(baseName + "Frameset", false));
            framesetCheck.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> v, Boolean oldV, Boolean newV) {
                    AppVariables.setUserConfigValue(baseName + "Frameset", framesetCheck.isSelected());
                }
            });
        ...
        try {
            ...
            if (framesetCheck.isSelected()) {
                ...
                if (framesetFile != null) {
                    ...
                    StringBuilder s;
                    String subPath = FileTools.filenameFilter(prefix) + "-frameset";
    ...
    public void exportNotes(Connection conn, Notebook book, String baseName) {
        ...
        try {
            ...
            File bookFile = null;
            if (framesetNavWriter != null) {
                File bookNavFile = new File(framesetNavFile.getParent() + File.separator + FileTools.filenameFilter(bookName) + "_nav.html");
                bookNavWriter = new FileWriter(bookNavFile, charset);
                writeHtmlHead(bookNavWriter, bookName);
                bookNavWriter.write(indent + "<BODY>\n");

                bookFile = new File(framesetNavFile.getParent() + File.separator + FileTools.filenameFilter(bookName) + ".html");
    ...
    protected void writeTexts(Connection conn, String bookName, Note note) {
        try {
            ...
            if (timeCheck.isSelected()) {
                textsWriter.write(DateTools.datetimeToString(note.getUpdateTime()) + "\n");
            }
