...
public class WebFavoritesExportController extends BaseTaskController {
    ...
    public WebFavoritesExportController() {
        baseTitle = AppVariables.message("FavoritesExport");
    }
    ...
    public void initControls() {
        try {
            ...
            iconCheck.setSelected(AppVariables.getUserConfigBoolean(baseName + "Icon", false));
            iconCheck.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> v, Boolean oldV, Boolean newV) {
                    AppVariables.setUserConfigValue(baseName + "Icon", iconCheck.isSelected());
                }
            });
    ...
    public void exportNode(Connection conn, TreeNode node, String baseName) {
        ...
        try {
            ...
            File nodeFile = null;
            if (framesetNavWriter != null) {
                File bookNavFile = new File(framesetNavFile.getParent() + File.separator + FileTools.filenameFilter(title) + "_nav.html");
                bookNavWriter = new FileWriter(bookNavFile, charset);
                writeHtmlHead(bookNavWriter, title);
                bookNavWriter.write(indent + "<BODY>\n");

                nodeFile = new File(framesetNavFile.getParent() + File.separator + FileTools.filenameFilter(title) + ".html");
