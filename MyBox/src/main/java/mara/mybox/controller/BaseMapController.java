...
public abstract class BaseMapController extends BaseController {
    ...
    public void initControls() {
        try {
            ...
            interval = AppVariables.getUserConfigInt(baseName + "Interval", 200);
                ...
                intervalSelector.getSelectionModel().selectedItemProperty().addListener(
                        (ObservableValue<? extends String> ov, String oldValue, String newValue) -> {
                            try {
                                ...
                                if (v > 0) {
                                    interval = v;
                                    AppVariables.setUserConfigInt(baseName + "Interval", interval);
...
//            MyBoxLog.debug(data);
    ...
    protected String snapName(boolean withFrame) {
        ...
        name += "_dpi" + dpi;
        return FileTools.filenameFilter(name);
    }
    ...
    protected void snapCurrentFrame() {
        ...
        File file = chooseSaveFile(AppVariables.getUserConfigPath(VisitHistoryTools.getPathKey(VisitHistory.FileType.Image)),
                filename, CommonFxValues.ImageExtensionFilter);
        if (file == null) {
            return;
        }
            ...
            task = new SingletonTask<Void>() {
                ...
                protected boolean handle() {
                    try {
                        ...
                        FileTools.writeFile(htmlFile, html, Charset.forName("utf-8"));
