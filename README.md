...
可以下载exe包，无需java环境、无需安装、解包可用： [MyBox-2.11-exe.zip](https://github.com/Mararsh/MyBox/releases/download/v2.11/MyBox-2.11-exe.zip) 

在已安装JRE或者JDK的环境下(最好是版本8)，可以下载jar包 [MyBox-2.11-jar.zip](https://github.com/Mararsh/MyBox/releases/download/v2.11/MyBox-2.11-jar.zip) ，执行以下命令来启动程序：
...
[用户手册：https://github.com/Mararsh/MyBox/releases/download/v2.11/MyBox-UserGuide-2.11.pdf](https://github.com/Mararsh/MyBox/releases/download/v2.11/MyBox-UserGuide-2.11.pdf)
...
2. 图像工具：
	...
	O.像素计算器
3. 文件和目录工具：
	A. 网页编辑器：富文本方式编辑网页，或直接编辑html代码。
	B. 网页浏览器：加载网址；可同步网页编辑器；可把网页完整内容保存为一张图片。
	C. 文本编辑器。
	D. 目录文件重命名，包含文件名和排序的选项。被重命名的文件可以全部恢复或者指定恢复原来的名字。
	E. 目录同步，包含复制子目录、新文件、特定时间以后已修改文件、原文件属性，以及删除源目录不存在文件和目录，等选项。
	F. 整理文件，将文件按修改时间或者生成时间重新归类在新目录下。此功能可用于处理照片、游戏截图、和系统日志等需要按时间归档的批量文件。
...
## 开发日志
```
2018-09-06 版本2.11 图片的合并，支持排列选项、背景颜色、间隔、边沿、和尺寸选项。网页浏览器，同步网页编辑器，把网页完整内容保存为一张图片。图片处理：阴影、圆角、加边。
确保大图片处理的正确性和性能。
