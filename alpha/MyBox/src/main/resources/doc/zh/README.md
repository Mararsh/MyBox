...
* 编辑表数据：单击开始编辑，失焦提交修改；类型检验；行的复选框。       
* 编辑数据：“表”为主编辑模式，“文本”为辅编辑模式；统一数据管理界面；处理逻辑从界面移至对象。     
* 图像：改进图像文件的读取方法；在后端加载缩略图；颜色选择窗口可以直接加载示例调色盘。                         
* 界面：网页和弹出窗口的样式可所有界面共享；查找/替换的选项可所有界面共享；截图当前窗口或编辑内容。                  
...
在我的Mac环境（OS X 10.11.6/Darwin 15.6.0）下,openJDK 17无法启动，所以MyBox的mac版本基于openJDK 16。
...
2. 数据粘贴板
	- 数据记录可以在在MyBox粘贴板中增删改。
	- 数据保存在MyBox内部目录下的文件中。
	- 删除数据粘贴板的记录将会同时删除内部文件。
3. 矩阵
	- 数据记录可以在矩阵管理器中增删改。
	- 数据保存在数据库中。
