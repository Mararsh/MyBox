...
public class ControlDataSplit extends BaseController {
    ...
    public void setParameters(String name) {
        try {
            ...
            from = UserConfig.getDouble(baseName + "From", -1);
            fromInput.setText(from + "");

            to = UserConfig.getDouble(baseName + "To", 1);
            toInput.setText(to + "");

            number = UserConfig.getInt(baseName + "Number", 10);
    ...
    public boolean checkInputs() {
        try {
            ...
            try {
                to = Double.valueOf(toInput.getText().trim());
            ...
            if (byInterval) {
                ...
                intervalInput.setStyle(null);
                try {
                    number = Integer.valueOf(numberInput.getText().trim());
                    numberInput.setStyle(null);
                } catch (Exception e) {
                    numberInput.setStyle(UserConfig.badStyle());
                    popError(message("InvalidParamter") + ": " + message("NumberOfData"));
                    return false;
                }
            }
