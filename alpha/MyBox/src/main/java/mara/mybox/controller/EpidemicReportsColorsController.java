...
public class EpidemicReportsColorsController extends BaseController {
        ...
        try {
            ...
            for (int i = 0; i < locations.size(); i++) {
                ...
                FXMLLoader fxmlLoader = new FXMLLoader(
                        WindowTools.class.getResource(Fxmls.ColorSetFxml), AppVariables.currentBundle);
                Pane pane = fxmlLoader.load();
                ColorSet controller = (ColorSet) fxmlLoader.getController();
                controller.init(this, controllerName, Color.web(FxColorTools.randomRGB()));

                String color = controller.rgb();
                if (colors.contains(color)) {
                    color = FxColorTools.randomRGBExcept(colors);
    ...
    public void randomSourcesColor() {
        List<String> colors = FxColorTools.randomRGB(4);
    ...
    public void randomValuesColors() {
        List<String> colors = FxColorTools.randomRGB(14);
    ...
    public void randomLocationsDataColors() {
        ...
        List<String> colors = FxColorTools.randomRGB(size);
    ...
    public String rgb(String name) {
        try {
            ...
            if (Languages.message("Confirmed").equals(name)) {
                return confirmedColorSetController.rgb();

            } else if (Languages.message("Healed").equals(name)) {
                return healedColorSetController.rgb();

            } else if (Languages.message("Dead").equals(name)) {
                return deadColorSetController.rgb();

            } else if (Languages.message("IncreasedConfirmed").equals(name)) {
                return IncreasedConfirmedColorSetController.rgb();

            } else if (Languages.message("IncreasedHealed").equals(name)) {
                return IncreasedHealedColorSetController.rgb();

            } else if (Languages.message("IncreasedDead").equals(name)) {
                return IncreasedDeadColorSetController.rgb();

            } else if (Languages.message("HealedConfirmedPermillage").equals(name)) {
                return HealedConfirmedPermillageColorSetController.rgb();

            } else if (Languages.message("DeadConfirmedPermillage").equals(name)) {
                return DeadConfirmedPermillageColorSetController.rgb();

            } else if (Languages.message("ConfirmedPopulationPermillage").equals(name)) {
                return ConfirmedPopulationPermillageColorSetController.rgb();

            } else if (Languages.message("HealedPopulationPermillage").equals(name)) {
                return HealedPopulationPermillageColorSetController.rgb();

            } else if (Languages.message("DeadPopulationPermillage").equals(name)) {
                return DeadPopulationPermillageColorSetController.rgb();

            } else if (Languages.message("ConfirmedAreaPermillage").equals(name)) {
                return ConfirmedAreaPermillageColorSetController.rgb();

            } else if (Languages.message("HealedAreaPermillage").equals(name)) {
                return HealedAreaPermillageColorSetController.rgb();

            } else if (Languages.message("DeadAreaPermillage").equals(name)) {
