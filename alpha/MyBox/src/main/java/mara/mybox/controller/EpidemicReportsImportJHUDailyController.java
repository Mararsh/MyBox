...
public class EpidemicReportsImportJHUDailyController extends EpidemicReportsImportController {
    ...
    public EpidemicReportsImportJHUDailyController() {
        baseTitle = Languages.message("ImportEpidemicReportJHUDaily");
    }
    ...
    public long importFile(Connection conn, File file) {
        ...
        try ( CSVParser parser = CSVParser.parse(convertedFile, TextFileTools.charset(convertedFile), CsvTools.csvFormat());
            ...
            for (CSVRecord record : parser) {
                ...
                try {
                    ...
                    if (city != null && !city.isBlank()) {
                        levelValue = Languages.message("City");
                        ...
                        updateLogs(lineCount + " " + Languages.message("InvalidFormat") + " " + Languages.message("MissLocation"), true);
                        break;
                    }
                    ...
                    if (time.contains("/")) {
                        ...
                        date = DateTools.encodeDate(time.replace("T", " "), true);
                    }
                    if (date == null) {
                        updateLogs(lineCount + " " + Languages.message("InvalidFormat") + " " + Languages.message("MissTime"), true);
                        break;
                    ...
                    date = DateTools.encodeDate(DateTools.datetimeToString(date, "yyyy-MM-dd") + EpidemicReport.COVID19TIME, true);
                    ...
                    if (existed) {
                        if (update == null) {
                            ...
                            if (verboseCheck.isSelected() || (skipCount % 100 == 0)) {
                                updateLogs(Languages.message("Skip") + ": " + info, true);
                            }
                            continue;
                        }
