...
public class ShapeTools {
    ...
    public static Image drawShape(FxTask task, Image image, DoubleShape shape, ShapeStyle style, PixelsBlend blender) {
        ...
        BufferedImage target = mara.mybox.bufferedimage.ShapeTools.drawShape(task, source, shape, style, blender);
    ...
    public static Image drawErase(FxTask task, Image image, DoublePolylines penData, ShapeStyle style) {
        ...
        BufferedImage source = SwingFXUtils.fromFXImage(image, null);
        BufferedImage target = mara.mybox.bufferedimage.ShapeTools.drawErase(task, source, penData, style);
        if (target == null || (task != null && !task.isWorking())) {
            return null;
        }
        Image newImage = SwingFXUtils.toFXImage(target, null);
    ...
    public static Image drawMosaic(FxTask task, Image image, DoublePolylines penData,
        ...
        BufferedImage source = SwingFXUtils.fromFXImage(image, null);
        BufferedImage target = mara.mybox.bufferedimage.ShapeTools.drawMosaic(task, source, penData, mosaicType, strokeWidth, intensity);
        if (target == null || (task != null && !task.isWorking())) {
            return null;
        }
        Image newImage = SwingFXUtils.toFXImage(target, null);
